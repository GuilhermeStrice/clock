...
class Config
{
    ...
    port_a_direction: number = 0x00;
    port_b_direction: number = 0x00;
    port_a_value: number = 0x00;
    port_b_value: number = 0x00;
    port_a_pullup: number = 0x00;
    port_b_pullup: number = 0x00;
    port_a_polarity: number = 0x00;
    port_b_polarity: number = 0x00;
    port_a_latch_value: number = 0x00;
    port_b_latch_value: number = 0x00;
...
export class MCP23017
{
    ...
    constructor(smbus: number, address: number)
    {
        this.smbus = smbus;
        ...
        this.bus.writeByteSync(this.config.ioaddress, IOCON, this.config.ioconfig);

        // Read port a, b
        this.config.port_a_direction = this.bus.readByteSync(this.config.ioaddress, IODIRA);
        this.config.port_b_direction = this.bus.readByteSync(this.config.ioaddress, IODIRB);

        this.config.port_a_value = this.bus.readByteSync(this.config.ioaddress, GPIOA);
        this.config.port_b_value = this.bus.readByteSync(this.config.ioaddress, GPIOB);

        this.config.port_a_latch_value = this.bus.readByteSync(this.config.ioaddress, OLATA);
        this.config.port_b_latch_value = this.bus.readByteSync(this.config.ioaddress, OLATA);

        this.config.port_a_pullup = this.bus.readByteSync(this.config.ioaddress, GPPUA);
        this.config.port_b_pullup = this.bus.readByteSync(this.config.ioaddress, GPPUB);

        this.config.port_a_polarity = this.bus.readByteSync(this.config.ioaddress, IPOLA);
        this.config.port_b_polarity = this.bus.readByteSync(this.config.ioaddress, IPOLB);
    }
     ...
     * @returns {number} Bit
     */
    checkBit(byte: number, bit: number): number
    {
     ...
     * @returns {number} Byte
     */
    updateByte(byte: number, bit: number, value: number): number
    {
        if (value == 0) 
            return byte & ~(1 << bit);
	    return byte | (1 << bit);
     ...
     */
    setPinDirection(pin: number, direction: number)
    {
        if (pin < 8) 
        {
            this.config.port_a_direction = this.updateByte(this.config.port_a_direction, pin, direction);
            this.bus.writeByteSync(this.config.ioaddress, IODIRA, this.config.port_a_direction);
        } 
        else 
        {
            this.config.port_b_direction = this.updateByte(this.config.port_b_direction, pin - 8, direction);
            this.bus.writeByteSync(this.config.ioaddress, IODIRB, this.config.port_b_direction);
        }
    }
     ...
     */
    setPortDirection(port: number, direction: number)
    {
        if (port == 0) 
        {
            this.bus.writeByteSync(this.config.ioaddress, IODIRA, direction);
            this.config.port_a_direction = direction;
        } 
        else 
        {
            this.bus.writeByteSync(this.config.ioaddress, IODIRB, direction);
     ...
     */
    setPinPullup(pin: number, value: number)
    {
        if (pin < 8) 
        {
            this.config.port_a_pullup = this.updateByte(this.config.port_a_pullup, pin, value);
            this.bus.writeByteSync(this.config.ioaddress, GPPUA, this.config.port_a_pullup);
        } 
        else 
        {
            this.config.port_b_pullup = this.updateByte(this.config.port_a_pullup, pin - 8, value);
            this.bus.writeByteSync(this.config.ioaddress, GPPUB, this.config.port_b_pullup);
        }
    }
     ...
     */
    getPinPullup(pin: number) : number
    {
        if (pin < 8) 
        {
            this.config.port_a_pullup = this.bus.readByteSync(this.config.ioaddress, GPPUA);
     ...
     * @param value Value
     */
    setPortPullups(port: number, value: number)
    {
        if (port == 0) 
        {
            this.config.port_a_pullup = value;
            this.bus.writeByteSync(this.config.ioaddress, GPPUA, value);
        } 
        else 
        {
            this.config.port_b_pullup = value;
            this.bus.writeByteSync(this.config.ioaddress, GPPUB, value);
        }
    }
     ...
     */
    getPortPullups(port: number) : number
    {
        if (port == 0) 
        {
            this.config.port_a_pullup = this.bus.readByteSync(this.config.ioaddress, GPPUA);
    ...
    writePin(pin: number, value: number)
    {
        if (pin < 8) 
        {
            ...
            this.bus.writeByteSync(this.config.ioaddress, GPIOA, this.config.port_a_value);
        } 
        else 
        {
            this.config.port_b_value = this.updateByte(this.config.port_b_value, pin - 8, value);
            this.bus.writeByteSync(this.config.ioaddress, GPIOB, this.config.port_b_value);
        }
    }
    ...
    writePort(port: number, value: number)
    {
        if (port == 0) 
        {
            this.bus.writeByteSync(this.config.ioaddress, GPIOA, value);
            this.config.port_a_value = value;
        } 
        else
        {
            this.bus.writeByteSync(this.config.ioaddress, GPIOB, value);
     ...
     */
    invertPin(pin: number, polarity: number)
    {
        if (pin < 8)
        {
            this.config.port_a_polarity = this.updateByte(this.config.port_a_polarity, pin, polarity);
            this.bus.writeByteSync(this.config.ioaddress, IPOLA, this.config.port_a_polarity);
        } 
        else 
        {
            this.config.port_b_polarity = this.updateByte(this.config.port_b_polarity, pin - 8, polarity);
            this.bus.writeByteSync(this.config.ioaddress, IPOLB, this.config.port_b_polarity);
        }
    }
     ...
     * @param polarity Polarity of the input pin, 0 = same logic state, 1 = inverted logic state
     */
    invertPort(port: number, polarity: number)
    {
        if (port == 0) 
        {
            this.bus.writeByteSync(this.config.ioaddress, IPOLA, polarity);
            this.config.port_a_polarity = polarity
        } 
        else 
        {
            this.bus.writeByteSync(this.config.ioaddress, IPOLB, polarity);
     ...
     */
    setInterruptPolarity(value: number)
    {
        if (value == 0) 
        {
     ...
     */
    setInterruptType(port: number, value: number)
    {
        if (port == 0) 
        {
            this.bus.writeByteSync(this.config.ioaddress, INTCONA, value);
        } 
        else 
        {
            this.bus.writeByteSync(this.config.ioaddress, INTCONB, value);
        }
    }
    ...
    readPinOutputLatch(pin: number)
    {
        ...
        {
            this.config.port_b_latch_value = this.bus.readByteSync(this.config.ioaddress, OLATB);
     ...
     * @returns {number} Value is a number between 0 and 255 or 0x00 and 0xFF
     */
    readPortOutputLatches(port: number)
    {
        if (port == 0) 
        {
            this.config.port_a_latch_value = this.bus.readByteSync(this.config.ioaddress, OLATA);
     ...
     */
    setInterruptDefaults(port: number, value: number)
    {
        if (port == 0) 
        {
            this.bus.writeByteSync(this.config.ioaddress, DEFVALA, value);
        } 
        else 
        {
            this.bus.writeByteSync(this.config.ioaddress, DEFVALB, value);
        }
    }
     ...
     * @param value Value is a number between 0 and 255 or 0x00 and 0xFF
     */
    setInterruptOnPort(port: number, value: number)
    {
        if (port == 0) 
        {
            this.bus.writeByteSync(this.config.ioaddress, GPINTENA, value);
            this.config.inta = value
        } 
        else 
        {
            this.bus.writeByteSync(this.config.ioaddress, GPINTENB, value);
     ...
     */
    setInterruptOnPin(pin: number, value: number)
    {
        if (pin < 8) 
        {
            this.config.inta = this.updateByte(this.config.inta, pin, value);
            this.bus.writeByteSync(this.config.ioaddress, GPINTENA, this.config.inta);
        } 
        else 
        {
            this.config.intb = this.updateByte(this.config.intb, pin - 8, value);
            this.bus.writeByteSync(this.config.ioaddress, GPINTENB, this.config.intb);
        }
    }
     ...
     */
    readInterruptStatus(port: number)
    {
        if (port == 0) 
        {
            return this.bus.readByteSync(this.config.ioaddress, INTFA);
        } 
        else 
        {
            return this.bus.readByteSync(this.config.ioaddress, INTFB);
        }
    }
     ...
     */
    readInterruptCapture(port: number)
    {
        if (port == 0) 
        {
            return this.bus.readByteSync(this.config.ioaddress, INTCAPA);
        } 
        else 
        {
            return this.bus.readByteSync(this.config.ioaddress, INTCAPB);
        }
    }
